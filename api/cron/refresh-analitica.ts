/**
 * api/cron/refresh-analitica.ts — Frente I.1 · refresh matview cada 1 h
 *
 * Vercel cron: { path: "/api/cron/refresh-analitica", schedule: "0 * * * *" }
 * Auth: Bearer ${CRON_SECRET} (también acepta GET para invoke manual).
 *
 * Llama RPC refresh_mv_analitica_inventario() definida en migration 015.
...
export default async function handler(
  ...
  try {
    const { error } = await supa.rpc('refresh_mv_analitica_inventario');
    ...
    if (err?.code === '42883') {
      ...
      res.status(503).json({ ok: false, error: 'function_pending_migration_015' });
      return;
    }
    console.error('[cron/refresh-analitica]', err?.message ?? e);
