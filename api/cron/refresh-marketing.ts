/**
 * api/cron/refresh-marketing.ts — Frente I.3 · refresh matview cada 1 h
 *
 * Vercel cron: { path: "/api/cron/refresh-marketing", schedule: "45 * * * *" }
 *   (offset min 45: distinto de refresh-clinica/analitica en min 0/30 y de
 *    pull-gcal en 15/45 — comparte minuto con pull-gcal pero distinto recurso).
 * Auth: Bearer ${CRON_SECRET} (también acepta GET para invoke manual).
 *
 * Llama RPC refresh_mv_analitica_marketing() definida en migration 026.
...
export default async function handler(
  ...
  try {
    const { error } = await supa.rpc('refresh_mv_analitica_marketing');
    ...
    if (err?.code === '42883') {
      res.status(503).json({ ok: false, error: 'function_pending_migration_026' });
      return;
    }
    console.error('[cron/refresh-marketing]', err?.message ?? e);
